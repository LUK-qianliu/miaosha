...
public class SnowFlowUtil {
    ...
    public static class Factory {
        ...
        public SnowFlowUtil create(long idcId, long machineId) {
            return new SnowFlowUtil(this.idcBitNum, this.machineBitNum, idcId, machineId);
        }
    }
    ...
    /**
     *
     * @param idcBitNum 数据中心占用的位数，默认5
     * @param machineBitNum 机器标识占用的位数，默认5
     * @param idcId 数据中心的id
     * @param machineId 机器id
     */
    private SnowFlowUtil(int idcBitNum, int machineBitNum, long idcId, long machineId) {
...
//                throw new IllegalStateException("sequence over flow");
    ...
    public static void main(String[] args) {
        SnowFlowUtil snowFlowUtil = new Factory().create(5, 4);
        for (int i = 0; i <10 ; i++) {
            System.out.println(snowFlowUtil.nextId());
        }
    }
